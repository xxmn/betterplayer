...
class BPDashUtils {
  static Future<BPAsmsDataHolder> parse(String data, String masterPlaylistUrl) async {
    ...
    try {
      ...
      BPUtils.log("Exception on dash parse: $exception");
    }
    return BPAsmsDataHolder(tracks: tracks, audios: audios, subtitles: subtitles);
  }

  static List<BPAsmsTrack> parseVideo(XmlElement node) {
    ...
    representations.forEach((representation) {
      ...
      tracks.add(BPAsmsTrack(id, width, height, bitrate, frameRate, codecs, mimeType));
    });

    return tracks;
  }

  static BPAsmsAudioTrack parseAudio(XmlElement node, int index) {
  ...
  static BPAsmsSubtitle parseSubtitle(String masterPlaylistUrl, XmlElement node) {
    final String segmentAlignmentStr = node.getAttribute('segmentAlignment') ?? '';
